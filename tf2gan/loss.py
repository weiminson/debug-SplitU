...
def get_wgan_losses_fn():
    ...
    def g_loss_fn(f_logit):
        #tf.print('f logit', f_logit.shape,  output_stream=sys.stdout)
        f_loss = - tf.reduce_mean(f_logit)
        #tf.print('f loss shape', f_loss.shape,  output_stream=sys.stdout)
        #tf.print('f loss', f_loss,  output_stream=sys.stdout)
        return f_loss
